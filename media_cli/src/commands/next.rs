...
pub fn run(
    ...
    let entry = match title {
        ...
        None => {
            match most_recent_in_progress(entries) {
                Some(e) => e,
                None => {
        ...
        MediaEntry::Show(s) => {
            ...
            let ep_rel = if let Some(spec) = episode {
                let (season, ep_num) = parse_ep_spec(spec)
                    .ok_or_else(|| format!("couldn't parse episode \"{spec}\" — use format s01e04"))?;
                s.all_episodes()
            ...
            let title_str = entry_display_title(entry);
            println!("  {} {}  —  {}", st.bold("Playing"), st.cyan(title_str), ep.display_label());
