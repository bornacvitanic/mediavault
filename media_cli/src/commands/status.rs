...
pub fn run(entries: &[MediaEntry], json: bool) -> Result<(), String> {
    ...
    let total_movies = entries
        .iter()
        .filter(|e| matches!(e, MediaEntry::Movie(_)))
        .count();
    let total_shows = entries
        .iter()
        .filter(|e| matches!(e, MediaEntry::Show(_)))
        .count();

    // ── In-progress shows ─────────────────────────────────────────────────────
    let in_progress: Vec<&MediaEntry> = entries
        .iter()
        .filter(|e| match e {
            MediaEntry::Show(s) => {
    ...
    let recent_movies: Vec<&MediaEntry> = entries
        .iter()
        .filter(|e| matches!(e, MediaEntry::Movie(m) if m.state.watched))
        .collect();

    // ── Unwatched entries ─────────────────────────────────────────────────────
    let unwatched_count = entries
        .iter()
        .filter(|e| match e {
    ...
    if in_progress.is_empty() && recent_movies.is_empty() {
        ...
        println!();
        println!(
            "  {}  run `mediavault-cli ls` to browse the library",
            st.dim("hint")
        );
        println!();
        return Ok(());
...
fn run_json(entries: &[MediaEntry]) -> Result<(), String> {
    let in_progress: Vec<serde_json::Value> = entries
        .iter()
        .filter_map(|e| {
            match e {
                MediaEntry::Show(s) => {
